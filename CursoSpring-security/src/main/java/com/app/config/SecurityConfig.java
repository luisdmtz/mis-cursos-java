    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception { // EL HttpSecurity PASA POR MEDIO DE LOS FILTROS
        return httpSecurity
                ...
                .authorizeHttpRequests(http -> {
                    ...
                    http.requestMatchers(HttpMethod.GET, "/auth/get").permitAll(); // ACCESO A TODOS PUBLICO
                    
                    // Configurar los endpoints privados - POR->ROL: ADMIN, DEVELOPER
                    http.requestMatchers(HttpMethod.POST, "/auth/post").hasAnyRole("ADMIN", "DEVELOPER"); // ACCESO POR ROL EJEMPLO: "ADMIN", "DEVELOPER"   
                    http.requestMatchers(HttpMethod.GET, "/auth/hello").hasAnyRole("USER"); //ACCESO POR ROL: EJEMPLO USER
                    
                    // Configurar los endpoints privados - POR->PERMISOS: REFACTOR, READ, CREATE, UPDATE
                    http.requestMatchers(HttpMethod.PATCH, "/auth/patch").hasAuthority("REFACTOR"); // ACCESO POR PERMISOS EJEMPLO: "REFACTOR"
                    http.requestMatchers(HttpMethod.POST, "/auth/postper").hasAnyAuthority("CREATE","READ"); //ACCESO POR PERMISOS, EJEMPLO: UPDATE
